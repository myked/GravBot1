...
- **`FORWARD_SIGN` (in `src/main.cpp`) decouples "which end is front" from the tuned PID/encoder/PWM chain.** The HC-SR04 was remounted from the original front to the opposite end of the chassis; "forward" (F, positive typed speed, RC `U`) now needs to drive toward that end. Rather than touching the encoder sign convention or PID (all validated, tuned, and documented under STABILITY FIXES in `main.cpp`), a single `-1` constant is applied only where `targetSpeedMMs`/`turnRateMMs` convert into `leftTargetMMs`/`rightTargetMMs`. It's applied identically to the speed and turn terms because redefining "front" is a 180° reinterpretation of the whole reference frame — that single sign flip correctly reverses forward/back *and* swaps which physical side responds to left/right at the same time. `handleRcChar()`, the typed-speed parser, and the obstacle-stop condition (`targetSpeedMMs > 0`) are all untouched and still mean what they say. The `LEFT/RIGHT FRONT/REAR` encoder pin labels in the wiring table are physical axle positions, unrelated to this — they don't need to change if the sensor moves again.
